...
Example tests might prove that one sample dataset works; PBT proves that your logic is correct for *all* datasets within realistic bounds. `spark-proof` brings this mindset to PySpark.
...
| **Debuggable failures** | When a property fails, the test framework automatically "shrinks" the data to the smallest example that still reproduces the failure. You get a tiny handful of rows that expose the issue. For example, "two identical keys on consecutive dates cause rank duplication". This makes failure analysis faster and keeps test feedback loops tight. |
